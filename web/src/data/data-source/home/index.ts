import type { Link, LinkList, LinkReport, NewLink, ResponseError } from "@/data/models/link";
...
export default class HomeDataSource {
    ...
    public getLinks = (): Promise<LinkList> => {
        ...
        return this.httpClient.get({ url, headers: { 'Content-Type': 'application/json' } }).then((response): LinkList => {
            const { body } = response
    ...
    public getLinksReport = (): Promise<LinkReport | ResponseError> => {
        ...
        return this.httpClient.get({ url, headers: { 'Content-Type': 'application/json' } }).then((response): LinkReport | ResponseError => {
            const { body } = response
    ...
    public deleteLink = (parameter: NewLink['shortUrl']): Promise<{ message: string }> => {
        const url = `${enviroment.beUrl}/link/${parameter}`;

        return this.httpClient.delete({ url, headers: { 'Content-Type': 'application/json' } }).then((response): { message: string } => {
            const { body } = response
